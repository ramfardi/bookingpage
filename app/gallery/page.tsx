...
return (
  ...
  <>
    {/* ---------------- CLIPS SECTION ---------------- */}
    {gallery.some((url) => isVideoUrl(normalizeGoogleDriveImage(url))) && (
      <section className="mb-20">
        <div className="text-center mb-10">
          ...
          <p className="mt-4 text-lg italic text-gray-600">
            Watch recent highlights and visual moments.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {gallery
            .filter((url) => isVideoUrl(normalizeGoogleDriveImage(url)))
            .map((url, index) => {
              const videoUrl = normalizeGoogleDriveImage(url);
              ...
              return (
                ...
                >
                  <video
                    src={videoUrl}
                    className="w-full h-[420px] rounded-[1.5rem] object-cover"
                    controls
                    playsInline
                  />
                </div>
    ...
    {gallery.some((url) => !isVideoUrl(normalizeGoogleDriveImage(url))) && (
      <section>
        <div className="text-center mb-10">
          <h2 className="text-4xl md:text-5xl font-black italic tracking-tight text-gray-900">
            Visual
            <span className="text-indigo-600"> Gallery</span>
          </h2>
